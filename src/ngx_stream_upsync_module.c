...
#define NGX_STREAM_UPSYNC_CONSUL_HEALTH        0x0003
#define NGX_STREAM_UPSYNC_ETCD                 0x0004
...
static ngx_int_t ngx_stream_upsync_consul_services_parse_json(
    void *upsync_server);
static ngx_int_t ngx_stream_upsync_consul_health_parse_json(
        void *upsync_server);
...
static ngx_upsync_conf_t  ngx_upsync_types[] = {
    ...
    { ngx_string("consul_health"),
      NGX_STREAM_UPSYNC_CONSUL_HEALTH,
      ngx_stream_upsync_send_handler,
      ngx_stream_upsync_recv_handler,
      ngx_stream_upsync_consul_parse_init,
      ngx_stream_upsync_consul_health_parse_json,
      ngx_stream_upsync_clean_event },
...
ngx_stream_upsync_check_index(ngx_stream_upsync_server_t *upsync_server)
{
    ...
    if (upsync_type_conf->upsync_type == NGX_STREAM_UPSYNC_CONSUL
        ...
        || upsync_type_conf->upsync_type == NGX_STREAM_UPSYNC_CONSUL_HEALTH) {
    ...
    ngx_stream_upsync_server_t *upsync_server)
{
    ...
    if (peers && servers->nelts >= 1) {
        ...
        for (i = 0; i < servers->nelts; i++) {
            ...
            index = ngx_stream_upstream_check_add_dynamic_peer(cycle->pool,
                                                             uscf, addrs);
...
#if (NGX_STREAM_UPSTREAM_CHECK)
                ngx_stream_upstream_check_delete_dynamic_peer(
...
#if (NGX_STREAM_UPSTREAM_CHECK)
            peer->check_index = server->addrs->.check_index;
#endif
            peer->next = peers->peer;
            peers->peer = peer;
...
static ngx_int_t
ngx_stream_upsync_consul_health_parse_json(void *data)
{
    ngx_buf_t                      *buf;
    ngx_stream_upsync_ctx_t          *ctx;
    ngx_stream_upsync_conf_t         *upstream_conf = NULL;
